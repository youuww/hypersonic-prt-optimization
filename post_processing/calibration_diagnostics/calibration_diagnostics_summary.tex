...
($\Prt = 0.566$) close to the DNS-measured boundary-layer average ($0.639$). The
velocity-space mismatch is real but is explained by a Reynolds-number/flow-state
difference and the Mach-14 strong viscous-interaction regime, not by a turbulence-model
failure. The chief actionable issue is incomplete energy-equation convergence in the
stored baseline run.
\end{abstract}
...
The ``Do No Harm'' plot ($u/U_\infty$ vs $y/\delta$) shows RANS and DNS velocity
profiles that do \emph{not} overlap. If the momentum field is wrong, then $\nu_t$ is
...
\texttt{plot\_calibration\_diagnostics.py}.
...
is sound. \textbf{Result: it does not.} RANS $\utau \approx \SI{14.7}{m/s}$ vs DNS
$\SI{67.6}{m/s}$; the RANS profile sits far above the log law.
...
log law; RANS (red/blue) is too steep because \utau{} is far too low for the velocity
field at this station. This motivated the forensic study in Section~\ref{sec:forensics}.}
\end{figure}
...
\midrule
Standard ($0.9$)        & 1.213 & $+1.170$ & 0.319 \\
Calibrated ($0.566$)    & 0.283 & $+0.033$ & 0.281 \\
\bottomrule
\end{tabular}
...
\label{sec:forensics}
% ============================================================
Because Check 1 failed, we localized the cause with \texttt{investigate\_wall\_shear.py},
mapping wall and boundary-layer quantities along the entire plate rather than at a single
station.
...
\bottomrule
\end{tabular}
\caption{Boundary-layer development of the baseline ($\Prt=0.9$) run.
At the analysis station ($x=\SI{1.5}{m}$) $Re_\tau = 182$, versus DNS $Re_\tau = 646$.}
\end{table}

\paragraph{Findings.}
\begin{itemize}[nosep]
  \item \textbf{Reynolds / flow-state mismatch.} At $x=\SI{1.5}{m}$, $Re_\tau=182$ vs DNS
        $646$: a low-Re, still-developing BL compared against a fully-developed one. The
        fixed $Re=\num{5e6}$ convention means RANS is not at the DNS Reynolds number.
  \item \textbf{Strong hypersonic viscous interaction.} Wall pressure is \SI{244}{Pa} vs
        freestream \SI{109}{Pa} ($2.2\times$), and $\delta_{99}=\SI{82}{mm}$ is $\sim 3.5\times$
        the incompressible turbulent estimate. This is the classic Mach-14
        strong-interaction regime---\emph{not} a clean zero-pressure-gradient plate
        (the DNS is ZPG).
  \item \textbf{No transition / no pathology.} $C_f(x)$ decays monotonically with no
        ...
        no relaminarization). The incompressible $C_f$ reference lines are not fair
        baselines at Mach 14 (compressibility legitimately lowers $C_f$).
  \item \textbf{Incomplete convergence (actionable).} After \num{15000} iterations,
        $\mathrm{rms}[\rho] = -6.1$ but $\mathrm{rms}[\rho E] = -0.70$: the \emph{energy}
        equation---which governs the temperature field we calibrate on---is barely
        converged.
\end{itemize}
...
transition). Right: boundary-layer growth $\delta_{99}(x)$, over-thick relative to
incompressible turbulent theory (expected for Mach 14).}
\end{figure}
...
\section{Verdict}
% ============================================================
The velocity-space mismatch is \textbf{real but explained}: it stems from a
Reynolds-number/flow-state difference and the Mach-14 strong viscous-interaction regime
(plus secondary incomplete energy convergence), \emph{not} from a turbulence-model
failure or a numerical pathology. Crucially, this \emph{reinforces} the methodology:
because the momentum field at a given station need not match a DNS profile at a different
Reynolds number, calibrating in the station-invariant, self-similar $T(u)$ space is the
correct and robust choice---and Check 2 (collapse) and Check 3 (physical \Prt{} recovery)
hold regardless. The \Prt{} result is therefore a genuine physical correction, not a
fudge factor.
...
\begin{enumerate}[nosep]
  \item \textbf{Converged baseline (highest value).} Re-run the $\Prt=0.9$ baseline to a
        converged energy residual ($\mathrm{rms}[\rho E] \lesssim -5$); regenerate Check 1.
  \item \textbf{Matched-Reynolds comparison.} Where feasible, run RANS at the DNS
        Reynolds number (or compare at matched $Re_\tau$) to remove the flow-state
        confound from the wall-unit comparison.
...
(RMSE $0.739$); DNS BL-avg $\Prt = 0.639$. RANS $\utau = \SI{14.7}{m/s}$ vs DNS
$\SI{67.6}{m/s}$; $Re_\tau = 182$ vs $646$; wall $p = \SI{244}{Pa}$ vs freestream
$\SI{109}{Pa}$. Convergence: $\mathrm{rms}[\rho]=-6.1$, $\mathrm{rms}[\rho E]=-0.70$.
